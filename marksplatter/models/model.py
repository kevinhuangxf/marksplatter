...
class MarkSplatter(nn.Module):
    ...
    def blend(self, imgs, preds_w) -> torch.Tensor:
        ...
        if self.attenuation is not None:
            imgs_w, heatmap = self.attenuation(imgs, imgs_w)
        return imgs_w, heatmap

    def blend(self, imgs, preds_w, mask) -> torch.Tensor:
        imgs_w = self.scaling_i * imgs + self.scaling_w * preds_w * mask * 0.1
        return imgs_w, self.scaling_w * preds_w * mask * 0.1
    ...
    def embed(
        ...
        if position is not None:
            if position.shape[-2:] != (h, w):
                position = resize(position)
            position = position.to(imgs.device)
            preds_w = self.embedder(imgs_res, msgs.to(imgs.device), position)
        else:
        ...
        # heatmap = None
        if mask is not None:
            imgs_w, heatmap = self.blend(imgs, preds_w, mask)
        else:
            imgs_w, heatmap = self.blend(imgs, preds_w)
