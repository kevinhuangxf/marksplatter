...
def make_attn(in_channels, attn_type="vanilla"):
    assert attn_type in ["vanilla", "linear", "cross", "none"], f'attn_type {attn_type} unknown'
    ...
    if attn_type == "vanilla":
        ...
        return MVAttention(in_channels)
    else:
...
class VAEEncoder(nn.Module):
    ...
    def forward(self, x, pos=None):
        ...
        mean = pos.mean(dim=[1, 2, 3])
        std = pos.std(dim=[1, 2, 3])
        ...
        if pos is not None:
            hs_pos = [self.conv_in(pos)]
            for i_level in range(self.num_resolutions):
                for i_block in range(self.num_res_blocks):
                    h_pos = self.down[i_level].block[i_block](hs_pos[-1], temb)
                    if len(self.down[i_level].attn) > 0:
                        h_pos = self.down[i_level].attn[i_block](h_pos)
                    hs_pos.append(h_pos)
                if i_level != self.num_resolutions-1:
                    hs_pos.append(self.down[i_level].downsample(hs_pos[-1]))
        ...
        if pos is not None:
            h_pos = hs_pos[-1]
            h_pos = self.mid.block_1(h_pos, temb)
            # Calculate mean and variance of style features
            mean = h_pos.mean([2, 3], keepdim=True)
            std = h_pos.std([2, 3], keepdim=True) + 1e-8
            h = self.mid.attn_1(h, mean, std)
        else:
            h = self.mid.attn_1(h)
